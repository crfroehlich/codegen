...
namespace Services.API
{
    public partial class TermCategoryService : DocServiceBase
    {
        ...
        public object Get(TermCategorySearch request)
        {
                ...
                try 
                {
                    if(true != request.IgnoreCache) 
                    {
                        tryRet = Request.ToOptimizedResultUsingCache(Cache, cacheKey, new TimeSpan(0, DocResources.Settings.SessionTimeout, 0), () =>
                        {
                            ...
                            DocCacheClient.Set(cacheKey, ret, DocConstantModelName.TERMCATEGORY);
                            return ret;
                        });
                    }
                    if (tryRet == null)
                    {
                        _ExecSearch(request, (entities) => entities.ConvertFromEntityList<DocEntityTermCategory,TermCategory>(ret, Execute, requestCancel));
                        tryRet = ret;
                        DocCacheClient.Set(cacheKey, tryRet, DocConstantModelName.TERMCATEGORY);
                    }
                }
            ...
            DocCacheClient.SyncKeys(cacheKey, DocConstantModelName.TERMCATEGORY);
            return tryRet;
        }
        ...
        public object Get(TermCategory request)
        {
            ...
            if (true != request.IgnoreCache)
            {
                ret = Request.ToOptimizedResultUsingCache(Cache, cacheKey, new TimeSpan(0, DocResources.Settings.SessionTimeout, 0), () =>
                {
                    ...
                    DocCacheClient.Set(cacheKey, cachedRet, request.Id, DocConstantModelName.TERMCATEGORY);
                    return cachedRet;
                });
            }
            if(null == ret)
            {
                Execute.Run(s =>
                {
                    ...
                    DocCacheClient.Set(cacheKey, ret, request.Id, DocConstantModelName.TERMCATEGORY);
                });
            }
            DocCacheClient.SyncKeys(cacheKey, request.Id, DocConstantModelName.TERMCATEGORY);
            return ret;
        }
